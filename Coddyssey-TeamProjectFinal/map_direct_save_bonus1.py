...
def main(save_path, area_map, area_category: pd.DataFrame,
    ...
    apart_lines = [
        bfs_shortest_path_list_queue(
            ...
            0,
            15,
            block) for apart in end["apartment"]]
    apart_lines = [
        apart_line for apart_line in apart_lines if apart_line is not None]
    apart_lines.sort(key=lambda x: len(x))
    paths = pd.DataFrame({"x": [x for x, _ in apart_lines[0]], "y": [
                         y for _, y in apart_lines[0]]})
    ax.plot([x for x, _ in apart_lines[0]], [
            y for _, y in apart_lines[0]], c="red")

    start = apart_lines[0][-1]

    lines1 = bfs_shortest_path_list_queue(
        start, end["bandalgomcaffee"][0], 0, 15, 0, 15, block)
    lines2 = bfs_shortest_path_list_queue(
        start, end["bandalgomcaffee"][1], 0, 15, 0, 15, block)

    if len(lines1) <= len(lines2):
        ax.plot([x for x, _ in lines1], [y for _, y in lines1], c="red")
        paths.merge(pd.DataFrame(
            {"x": [x for x, _ in lines1], "y": [y for _, y in lines1]}))
        paths.to_csv("res/home_to_cafe_bonus.csv", index=False)
    else:
        ax.plot([x for x, _ in lines2], [y for _, y in lines2], c="red")
        paths.merge(pd.DataFrame(
            {"x": [x for x, _ in lines2], "y": [y for _, y in lines2]}))
        paths.to_csv("res/home_to_cafe_bonus.csv", index=False)
...
if __name__ == "__main__":
    ...
    main("res/map_final_bonus.png", area_map, area_category, area_struct)
